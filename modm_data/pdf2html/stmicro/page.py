...
def areas_black_white(page) -> dict:
    ...
    if page.index < 3 and "DS" in page._doc._name:
...
def linesize_black_white(line) -> str:
    rsize = line.height
...
def linesize_blue_gray(line) -> str:
...
def colors_black_white(color) -> str:
    if 0xff <= color <= 0xff:
        return "black"
...
def colors_blue_gray(color) -> str:
    if 0xff <= color <= 0xff:
        return "black"
    if 0xffffffff <= color <= 0xffffffff:
        return "white"
...
class Page:
    ...
    def __init__(self, page):
        ...
        producer = page._doc.metadata.get("Producer", "").lower()
        ...
        if "blue_gray" in self._template:
            self._areas = areas_blue_gray(page)
            self._spacing = spacing_blue_gray(page)
            self._colors = colors_blue_gray
            self._line_size = linesize_blue_gray
        elif "black_white" in self._template:
            self._areas = areas_black_white(page)
            self._spacing = spacing_black_white(page)
            self._colors = colors_black_white
            self._line_size = linesize_black_white

        # Patches to detect the header cells correctly
        if ((self._doc.name == "DS12930-v1" and self._page.index in range(90, 106)) or
            (self._doc.name == "DS12931-v1" and self._page.index in range(89, 105))):
            self._spacing["th"] = 0.1
        if ((self._doc.name == "RM0453-v2" and self._page.index in [1354]) or
            (self._doc.name == "RM0456-v2" and self._page.index in [2881]) or
            (self._doc.name == "RM0456-v3" and self._page.index in [2880]) or
            (self._doc.name == "RM0461-v4" and self._page.index in [1246])):
            self._spacing["th"] = 0.5
        if ((self._doc.name == "RM0456-v2" and self._page.index in [3005])):
            self._spacing["th"] = 0.52

    def _text_in_area(self, name, max_length=100, check_length=True) -> str:
        if name not in self._areas: return ""
        text = ""
        areas = self._areas[name]
        if not isinstance(areas, list): areas = [areas]
        for area in areas:
            text += self._page.text_in_area(area, max_length)
    ...
    @cached_property
    def top(self) -> str:
        if self._page.index == 0:
            return "Cover"
        return self._text_in_area("top", check_length=False)

    def is_relevant(self) -> bool:
        if any(c in page.top for c in {"Contents", "List of ", "Index"}):
            return False
        return True
    ...
    def _charlines_filtered(self, area, predicate = None, rtol = None) -> list[CharLine]:
        ...
        for char in self._page.chars_in_area(area):
        ...
        for chars in origin_lines_y.values():
            ...
            line = CharLine(self, chars,
                            ...
                            sort_origin=self._page.height - origin)
        ...
        for next_line in bbox_lines[1:]:
            ...
            if ((current_line._sort_origin + rtol * height) >
                ...
                # if line.rotation or self._page.rotation:
    ...
    def content_ast(self) -> list:
        ...
        if "DS" in self._doc._name:
            ...
            order_page = next((item.page for item in self._doc.toc if item.level == 0 and
                               re.search("ordering +information|part +numbering", item.title, re.IGNORECASE)), -1)
            with_graphics = (order_page != self._page.index)
        ...
        Node("page", parent=first_leaf, xpos=first_leaf.xpos, number=self._page.number)
        return ast
    ...
    def _graphics_filtered(self, area) -> list:
        ...
        graphic_clusters = self._page.graphic_clusters(lambda p: large_area.contains(p.bbox), em/2)
        ...
        for captions in ycaptions:
            ...
            for ii, (phrase, chars) in enumerate(sorted(captions, key=lambda c: c[1][0].origin.x)):
                ...
                if self._template == "blue_gray":
                    ...
                    cchars = self._page.chars_in_area(cbbox)
                    while True:
                        nbbox = Rectangle(left, max(graphic[0].top, cbbox.bottom - height), right, top)
                        nchars = self._page.chars_in_area(nbbox)
                ...
                if "Figure" in phrase:
                    ...
                    if self._template == "blue_gray":
                        ...
                        while True:
                            ...
                            if not self._page.chars_in_area(gbbox):
                                break
        ...
        for gbbox, paths in graphic_clusters:
            ...
            if any(isinstance(p, Image) for p in paths):
                ...
                is_table = stroked_table_lines or all(
                    [any(p.isclose(pp) for pp in path.bbox.points) for p in path.points].count(True) >= len(path.points) * 2 / 3
                    for path in paths)
            ...
            if "table" in category:
                ...
                nchars = [c for c in self._page.chars_in_area(cbbox) if c.unicode not in {0x20, 0xa, 0xd}]
    ...
    def _format_ascii_char(self, line, char, with_ansi=True):
        rc = char.char
        cp = self._char_properties(line, char)
        if cp["superscript"]:
            rc = rc.translate(SUPERSCRIPT_TRANSLATION)

        if with_ansi:
            if cp["bold"]:
                rc = f"\033[1m{rc}\033[22m"
            if cp["italic"]:
                rc = f"\033[3m{rc}\033[23m"
            if cp["underline"]:
                rc = f"\033[4m{rc}\033[24m"
        return rc

    def _format_ascii_area(self, area: Rectangle, with_ansi=True) -> list:
        x_em = self._spacing["x_em"]
        objects = self._objects_filtered(area)
        lh_factor = self._spacing["lh"]

        lines = []
        ypos = objects[0].bbox.top
        for obj in objects:
            if isinstance(obj, (Table, Figure)):
                left = round((obj.bbox.left - area.left) / x_em)
                lines.append(textwrap.indent(obj.format_ascii(with_ansi), " " * left))
                ypos = obj.bbox.bottom

            elif isinstance(obj, CharLine):
                newlines = round((ypos - obj.origin) / (lh_factor * obj.height))
                lines.extend([""] * (min(newlines, 3) - 1))
                line = ""
                xchars = 0
                for cluster in obj.clusters:
                    spaces = round((cluster.bbox.left - area.left) / x_em) - xchars
                    line += " " * spaces
                    line += "".join(self._format_ascii_char(obj, char, with_ansi) for char in cluster.chars)
                    xchars += spaces + len(cluster.chars)
                lines.append(line.rstrip())
                ypos = obj.origin

        return lines

    def format_ascii(self, with_ansi=True) -> str:
        lines = []
        for area in self._areas["content"]:
            lines.extend(self._format_ascii_area(area, with_ansi))
        return "\n".join(lines)
    ...
    def __repr__(self) -> str:
        return f"StPage({self._page.number})"
