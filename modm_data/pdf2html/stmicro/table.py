...
class TableCell:
    ...
    def format_ascii(self, with_ansi: bool = False):
        def _fmt(line, char):
            return self._table._page._format_ascii_char(line, char, with_ansi=False)
        cell_text = []
        for line in self.lines:
            text = "".join(c.char for c in line.chars)
            if text.startswith("Res"):
                continue
            if line.rotation == 90:
                cell_text.append("\n".join(_fmt(line, c) for c in reversed(line.chars)))
            else:
                cell_text.append("".join(_fmt(line, c) for c in line.chars))
        return "\n".join(cell_text)
...
class Table:
        ...
        if self._cells is None:
            ...
            if self._type != "register":
                ...
                for yi in range(0 if y_header_pos == self.grid[1] else y_header_pos, self.grid[1]):
                    ...
                    chars = self._page._page.chars_in_area(bbox)
    ...
    def format_ascii(self, with_ansi: bool = True) -> str:
        # Imported down here, since it requires BeautifulSoup to be installed manually
        from dashtable import data2rst
        ttext = [["" for _ in range(self.grid[0])] for _ in range(self.grid[1])]
        spans = [[[p[0], p[1]] for p in c.positions] for c in self.cells if len(c.positions) > 1]
        for cell in self.cells:
            ttext[cell.y][cell.x] = cell.format_ascii(with_ansi)
        return data2rst(ttext, spans)
