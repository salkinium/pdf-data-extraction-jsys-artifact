...
class Point:
    ...
    @property
    def np(self) -> numpy.array:
        return numpy.array([self.x, self.y, 1])
...
class Line:
    ...
    def __init__(self, *r, width: float = None):
    ...
    @property
    def np(self) -> numpy.array:
        return numpy.array([(self.p0.x, self.p0.y), (self.p1.x, self.p1.y)])

    def __repr__(self) -> str:
        if self.width:
            return f"<{repr(self.p0)},{repr(self.p1)},{self.width:.1f}>"
...
class VLine(Line):
    ...
    def __repr__(self) -> str:
        ...
        if self.width:
            return f"<X{x}:{y0},{y1}|{self.width:.1f}>"
        return f"<X{x}:{y0},{y1}>"
...
class HLine(Line):
    ...
    def __repr__(self) -> str:
        ...
        if self.width:
            return f"<Y{y}:{x0},{x1}|{self.width:.1f}>"
        return f"<Y{y}:{x0},{x1}>"
...
class Transform:
    def __init__(self, *m):
        if isinstance(m[0], pp.FS_MATRIX):
            self._m = numpy.array([(m[0].a, m[0].b, m[0].e), (m[0].c, m[0].d, m[0].f), (0, 0, 1)])
        elif len(m) == 6:
            self._m = numpy.array([(m[0], m[1], m[4]), (m[2], m[3], m[5]), (0, 0, 1)])
        else:
            self._m = m

    def map(self, *v):
        if isinstance(v[0], Rectangle):
            p0 = self.map(v[0].p0)
            p1 = self.map(v[0].p1)
            return Rectangle(p0, p1)

        elif isinstance(v[0], Point):
            p = self._m @ v[0].np
            return Point(p[0], p[1])

        elif isinstance(v[0], Transform):
            return Transform(self._m * v[0]._m)

        return self.map(Point(v[0], v[1]))
